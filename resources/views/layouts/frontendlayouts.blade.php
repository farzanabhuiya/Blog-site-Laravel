...
<head>
	...
	<title>Blog WebSite - Minimal Blog & Magazine HTML Theme</title>
						...
						<div class="d-flex align-items-center">
							<div class="collapse navbar-collapse flex-grow-1">
								...
								<li class="nav-item  {{ Route:: is('frontend')?  'active' :''}}" >
										...
										<a class="nav-link dropdown-toggle" href="{{ route('category.all',$category->slug)}}">{{$category->title}}</a>
										<ul class="dropdown-menu">
											@foreach ($category->subcatagories as $subcatagory )
											<li><a class="dropdown-item" href="{{ route('category.all',$subcatagory->slug)}}">{{ $subcatagory->title}}</a></li>
											@endforeach
